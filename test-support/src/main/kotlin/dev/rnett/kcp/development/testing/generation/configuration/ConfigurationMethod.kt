...
public abstract class ConfigurationHost {
    public abstract fun configureTest(testInstance: AbstractKotlinCompilerTest, builder: NonGroupingPhaseTestConfigurationBuilder)

    public companion object {

        @JvmStatic
        public fun applyRuntimeConfiguration(test: AbstractKotlinCompilerTest, builder: NonGroupingPhaseTestConfigurationBuilder, vararg hosts: Class<out ConfigurationHost>) {
...
public class RuntimeConfigurationMethodModel(public val hosts: Set<KClass<out ConfigurationHost>>) : MethodModel<RuntimeConfigurationMethodModel>() {
    ...
    override fun imports(): Collection<Class<*>> {
        return super.imports() + hosts.map { it.java } + ConfigurationHost::class.java + NonGroupingPhaseTestConfigurationBuilder::class.java
    }
    ...
    public object Generator : MethodGenerator<RuntimeConfigurationMethodModel>() {
        ...
        override fun generateSignature(method: RuntimeConfigurationMethodModel, p: Printer) {
            p.println("@Override")
            p.print("public void configure(NonGroupingPhaseTestConfigurationBuilder builder)")
        }
